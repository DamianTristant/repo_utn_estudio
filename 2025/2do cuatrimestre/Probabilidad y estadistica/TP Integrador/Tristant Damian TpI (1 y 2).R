...
h <- hist(datos$`TIEMPO SEMANAL en HS. DEDIC. EST.`, breaks = 10, plot = FALSE)
brks <- h$breaks 

ggplot(data.frame(tiempo = datos$`TIEMPO SEMANAL en HS. DEDIC. EST.`), aes(x = tiempo)) +
  geom_histogram(breaks = brks, # Podríamos usar binwidth y forzar el ancho
                 closed = "right",         # igual que hist(): (a,b]
                 fill = "skyblue", color = "white") +
  labs(title = "Histograma Tiempo semanal en hs dedicados al estudio",
       x = "Tiempo (Horas)", y = "Frecuencia")


nivel_satis <- data.frame(Nivel_satisfa = nivel_satisfaccion , porcentaje = f_rel )

ggplot(nivel_satis, aes(x = "", y = porcentaje.tabla_satisfaccion, fill = Nivel_satisfa.Nivel_satisfaccion.Nivel_Satisfaccion)) + #fill = SO, hace que cada sector se pinte según la categoría
  geom_bar(stat = "identity", width = 1) +  # barras proporcionales
  coord_polar("y") +                        # convierte barras en sectores circulares
  labs(title = "Nivel Satisfaccion") +
